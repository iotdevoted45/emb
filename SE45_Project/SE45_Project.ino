...
void setup(void) {
    ...
    delay(100);    liftSetup();      // it will set lift on GND if it not
    delay(120);    CLOSE_PUSH();
    delay(200);    CLOSE_D1();       // if open it will close the inner door
    delay(150);    CLOSE_D2();       // if open it will close the outer door
...
mcpU7.writeRegister(MCP23017Register::GPIO_B, 0);
    // Serial.println (" :: This is while in Setup :: ");while(999);
}

////////////////////////////////////////////
//    it will set lift on GND After power on
void liftSetup(void){
  Serial.println(" :: Checking and Setup LIFT Going at GND :: ");
  for (long zz=0;zz<98000 ;zz++){
      int Delay = (zz < 8 )? 2999 : 820;
      Lift_ONE_DOWN(Delay);   Serial.println(zz);
...
void CLOSE_D1(void){
  ...
  mcpU7.writeRegister(MCP23017Register::GPIO_A, Motor_Door_1);  
  for(int S=0; S < (200*25); S++){
        LINE_X_FRD(0x20,0x00);      Serial.println(S);
        if(!digitalRead(IN_DOOR)){
          for(int SS=0; SS < (200*21); SS++) 
              LINE_X_RVC(0x30,0x10);   
          break;       
        }
  }  Serial.println(" :: Now Door1 Inner door is closed  :: ");
}
...
void CLOSE_D2(void){
  ...
  mcpU7.writeRegister(MCP23017Register::GPIO_A, Motor_Door_2);  
  for(int S=0; S < (200*28); S++){
        LINE_X_FRD(0x80,0x00);        Serial.println(S);
        if(!digitalRead(OT_DOOR)){
          for(int SS=0; SS < (200*26); SS++) 
            LINE_X_RVC(0xC0,0x40); 
          break;       
        } 
  }   Serial.println(" :: Now Door2 Outer door is closed  :: ");
}

////////////////////////////////////////////
//    if open it will close the outer door
void CLOSE_PUSH(void){
  Serial.println(" :: Checking and Closing the PUSH_PLAT at BAckside :: ");  
    mcpU7.writeRegister(MCP23017Register::GPIO_A, Motor_PUSH_0);  //Reset port B
    for(int S=0; S < (200*98); S++){
        LINE_X_RVC(0x0C,0x04);    Serial.println(S);
        if(!digitalRead(Push_limit))
          break;
    } Serial.println(" :: Now PUSH_PLAT is full back side  :: ");
}
...
void loop(void) {
  ...
  if(RCV_NEW_ORDER){          //  it will work on new order
      ...
      MOTOR_IN_D(1,16);    // open inner door
      delay(20);
      MOTOR_PUSH(1,93);    // pusing item out side 
      delay(200);
      MOTOR_PUSH(0,15);    // close bit 
      delay(20); 
      MOTOR_IN_D(0,16);    // closined the inner door 
      delay(20);
      MOTOR_OT_D(1,25);    // open outer door
      delay(20);
      MOTOR_PUSH(0,77);    // close all right
      serverUpdate(G_OrderID);
      delay(30000);        // waiting 2 min     
      MOTOR_OT_D(0,15);    // close outer door
      delay(3000);
      MOTOR_OT_D(0,10);    // close outer door
...
void MOTOR_PUSH(bool Dir, int Rot){             /// Lift will move up from GND to A locatiion
  if(Dir)
    Serial.println(" :: Pushing item in BOx :: ");
  else  
    Serial.println(" :: Push_Motor moving back Side :: ");    
    mcpU7.writeRegister(MCP23017Register::GPIO_A, Motor_PUSH_0);  //Reset port B
