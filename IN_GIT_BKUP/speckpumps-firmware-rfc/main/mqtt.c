...
const char mqtt_broker_uri[]= "mqtts://baduconnect.com:8883";
...
void start_mqtt(void)
{
	...
	ESP_LOGI(TAG, "Client ID: %s", mqttclientid);
	ESP_LOGI(TAG, "MQTT URI: %s", mqtturi);
	ESP_LOGI(TAG, "Username: %s", mqttusername);
	ESP_LOGI(TAG, "Pub channel:%s", mqttMsgPubChannel);
...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event)
{
	...
	switch(event->event_id){
		...
		case MQTT_EVENT_CONNECTED:
			...
			ESP_LOGI(TAG, "Subscribe to : [%s]", mqttTopicBuf);
		...
		case MQTT_EVENT_DISCONNECTED:
			...
			ESP_LOGI(TAG, "MQTT is disconnected \n");
			break;
	...
	while(1)
	{
		...
		if(xEventGroupGetBits(common_events_group) & (1 << enum_connected_to_mqtt))
		{
			if(pdTRUE == xQueueReceive(queue_mqtt_msg_to_server, &msg_to_publish, 0))
			{
				...
				{
					ESP_LOGE(TAG, "Publishing: [%s] on [%s] retain [%d]", mqttMsgPubChannel, mqttData, retain);
